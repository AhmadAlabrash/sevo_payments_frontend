...
function App() {
  ...
  async function getNameAndBalance() {
      const res = await axios.get(`https://sevopayments.onrender.com/getNameAndBalance`, {
    ...
    <div className="App">
      <Layout>
        <Header className="header">
          ...
          {isConnected ?
                   ( <Button type={"primary"} onClick={()=>disconnectAndSetNull()} style={{ backgroundColor: '#e4ace3', color: 'black' }}>Disconnect Wallet</Button>

          ) :        ( <Button type={"primary"} onClick={()=>connect() } style={{ backgroundColor: '#e4ace3', color: 'black' }} >Connect Wallet</Button>
        ...
        <Content className="content">
          ...
          :
          ( 
<div style={{ marginTop: '50px',  minHeight: 'calc(100vh - 50px)', display: 'flex', justifyContent: 'center', alignItems: 'center',fontSize: '24px', fontWeight: 'bold' }}>
